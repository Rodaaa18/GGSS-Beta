...
import {
  ...
  reloadItem,
} from "../../redux/actions/trabajosAnterioresActions";
import {
  cleanIdsDoc,
  documentacionDelEmpleado,
  getOneDocumento,
} from "../../redux/actions/documentacionActions";
import {
  addDetalleLicencia,
  addLicEmpleado,
  clearIdsLic,
...
import {
  cleanIdFam,
  getDAtosFamiliaresEmpleado,
} from "../../redux/actions/familiaActions";
import {
  addOneDomicilio,
  cleanIdsDom,
} from "../../redux/actions/domiciliosActions";
import {
  addDatosExtraPorEmpleado,
  cleanIdDe,
} from "../../redux/actions/extrasActions";
...
const Empleados = ({ tokenDef, setTokenDef, sePerfilesUSuario, loading }) => {
  ...
  const domiciliosEmpleados = useSelector(
    (state) => state.generalState.domicilios
  );
  const empleadoDomicilio = useSelector(
    (state) => state.domiciliosStates.domicilioEmpleado
  );
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://54.243.192.82/api/MostrarDatosFamiliarPorEmpleado/${empleadoUno?.iDempleado}`
      )
      .then((res) => {
    ...
    axios
      .get(
        `http://54.243.192.82/api/Documentacion/sp_DocumentacionDatosXIdEmpleado?IdEmpleado=${empleadoUno?.iDempleado}`
      )
      .then((res) => {
    ...
    axios
      .get(
        `http://54.243.192.82/api/MostrarDatosExtras/0,${empleadoUno?.iDempleado},1,`
      )
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://54.243.192.82/api/sp_DomiciliosDatosxIdEmpleado?IdEmpleado=${empleadoUno?.iDempleado}`
      )
      .then((res) => {
  ...
  const urlEmpleadoPorApellido = `http://54.243.192.82/api/Empleados?records=0&page=1&filter=${
    responses?.browser?.inputApellidoNombreBrowser
      ? responses?.browser?.inputApellidoNombreBrowser
      : null
  }&ordered=true`;
  const urlEmpleadoPorLegajo = `http://54.243.192.82/api/Empleados?records=0&page=1&legajo=${
    responses?.browser?.inpurLegajoBrowser
      ? responses?.browser?.inpurLegajoBrowser
      : null
  }&ordered=true`;
  const urlEmpleadoApYLegajo = `http://54.243.192.82/api/Empleados?records=0&page=1&filter=${
    responses?.browser?.inputApellidoNombreBrowser
      ? responses?.browser.inputApellidoNombreBrowser
      : null
  }&legajo=${
  ...
  const urlApeLegOrdered = `http://54.243.192.82/api/Empleados?records=10000&filter=${
    responses?.browser?.inputApellidoNombreBrowser
      ? responses?.browser?.inputApellidoNombreBrowser
      : null
  }&legajo=${
  ...
  async function getEmpleados() {
    if (responses?.browser.inputApellidoNombreBrowser) {
      ...
      responses?.browser.inputApellidoNombreBrowser &&
      responses?.browser.inpurLegajoBrowser &&
      responses?.browser.ordered
    ) {
      await axios
        .get({ method: "get", url: urlApeLegOrdered, timeout: 2000 })
        .then((res) => {
          dispatch(getEmployes(res.data.result));
        });
      return;
  ...
  const idsTrabajosAnterioresDelete = useSelector(
    (state) => state.trabajosAnteriores.ids
  );
  const documentacionDelte = useSelector(
    (state) => state.documentacionState.ids
  );
  const licenciasDelete = useSelector((state) => state.licenciasState.idsLic);
  const idDomiciliosArray = useSelector(
    (state) => state.domiciliosStates.idsDom
  );
  ...
  const urlTRabajoDelete =
    "http://54.243.192.82/api/TrabajosAnteriores?IdTrabajoAnterior=";
  ...
  async function deleteEmploye(id) {
    try {
      await axios
        .delete(`http://54.243.192.82/api/Empleados/${id}`)
        .then((res) => {
  ...
  async function deleteItems(objectRequest) {
    ...
    let bodyPetitionEmpleadoUpdate = {
      ...
      legajo: responses.formDatosPersonales?.numLegajo
        ? responses.formDatosPersonales?.numLegajo
        : empleadoUno.legajo,
      apellido: responses.formDatosPersonales?.apellidoInput
        ? responses.formDatosPersonales?.apellidoInput
        : empleadoUno.apellido,
      iDtipoDocumento: responses.formDatosPersonales?.dniSelected
        ? responses.formDatosPersonales?.dniSelected
        : empleadoUno.iDtipoDocumento,
      nroDocumento: responses.formDatosPersonales?.documentoInput
        ? responses.formDatosPersonales?.documentoInput
        : empleadoUno.nroDocumento,
      cuil: responses.formDatosPersonales?.inputCuil
        ? responses.formDatosPersonales?.inputCuil
        : empleadoUno.cuil,
      sexo: responses.formDatosPersonales?.inputSexo
        ? responses.formDatosPersonales?.inputSexo
        : empleadoUno.sexo,
      iDestadoCivil: responses.formDatosPersonales?.estadoCivilInput
        ? responses.formDatosPersonales?.estadoCivilInput
        : empleadoUno.iDestadoCivil,
      idNacionalidad: responses.formDatosPersonales?.nacionalidadesInput
        ? responses.formDatosPersonales?.nacionalidadesInput
        : empleadoUno.iDnacionalidad,
      fechaNacimiento: responses.formDatosPersonales?.inputDateNac
        ? responses.formDatosPersonales?.inputDateNac
        : empleadoUno.fechaNacimiento,
      iDEstudios: responses.formDatosPersonales?.estudiosInput
        ? responses.formDatosPersonales?.estudiosInput
        : empleadoUno.iDestudios,
      fechaIngreso: responses.formLiquidacion?.ingresoDateInput
        ? responses.formLiquidacion?.ingresoDateInput
        : empleadoUno.fechaIngreso,
      fechaEfectiva: responses.formLiquidacion?.inputDateEfectivo
        ? responses.formLiquidacion?.inputDateEfectivo
        : empleadoUno.fechaEfectiva,
      iDCategoria: responses.formLiquidacion?.inputCategoria
        ? responses.formLiquidacion?.inputCategoria
        : empleadoUno.iDcategoria,
      iDCargo: responses.formLiquidacion?.inputCargo
        ? responses.formLiquidacion?.inputCargo
        : empleadoUno.iDcargo,
      iDTareaDesempeñada: responses.formLiquidacion?.inputTareaDesempeñada
        ? responses.formLiquidacion?.inputTareaDesempeñada
        : empleadoUno.iDtareaDesempeñada,
      idCentrodeCosto: responses.formLiquidacion?.inputCentroCosto
        ? responses.formLiquidacion?.inputCentroCosto
        : empleadoUno.idCentrodeCosto,
      iDSectorDpto: responses.formLiquidacion?.inputSectorDepto
        ? responses.formLiquidacion?.inputSectorDepto
        : empleadoUno.iDsectorDpto,
      iDModoContratacion: responses.formLiquidacion?.inputModoCOntratacion
        ? responses.formLiquidacion?.inputModoCOntratacion
        : empleadoUno.iDmodoContratacion,
      iDModoLiquidacion: responses.formLiquidacion?.inputModoLiquidacion
        ? responses.formLiquidacion?.inputModoLiquidacion
        : empleadoUno.iDmodoLiquidacion,
      iDFormadePago: responses.formLiquidacion?.inputFormaDePago
        ? responses.formLiquidacion?.inputFormaDePago
        : empleadoUno.iDformadePago,
      idBanco: responses.formLiquidacion?.inputBanco
        ? responses.formLiquidacion?.inputBanco
        : empleadoUno.idbanco,
      nroCtaBanco: responses.formLiquidacion?.inputNumCta
        ? responses.formLiquidacion?.inputNumCta
        : empleadoUno.nroCtaBanco,
      cbu: responses.formLiquidacion?.inputCBU
        ? responses.formLiquidacion?.inputCBU
        : empleadoUno.cbu,
      iDLugardePago: responses.formLiquidacion?.inputLugaresDePago
        ? responses.formLiquidacion?.inputLugaresDePago
        : empleadoUno.iDlugardePago,
      iDAFJP: 1,
      idObraSocial: responses.formLiquidacion?.inputObraSocial
        ? responses.formLiquidacion?.inputObraSocial
        : empleadoUno.iDobraSocial,
      iDSindicato: responses.formLiquidacion?.sindicatosLiquidacion
        ? responses.formLiquidacion?.sindicatosLiquidacion
        : empleadoUno.iDsindicato,
      fechaEgreso: "2022-12-27T15:45:01.031Z",
      iDMotivoEgreso: 4,
      iDEsquema: responses.formLiquidacion?.selectOptionsId
        ? responses.formLiquidacion?.selectOptionsId
        : empleadoUno.iDesquema,
      iDEmpleador: responses.formLiquidacion?.inputEmpleadorLiquidacion
        ? responses.formLiquidacion?.inputEmpleadorLiquidacion
        : empleadoUno.iDempleador,
      nombres: responses.formDatosPersonales?.nombresInput
        ? responses.formDatosPersonales?.nombresInput
        : empleadoUno.nombres,
      idEstado: responses.formDatosPersonales?.estadosEmpleados
        ? responses.formDatosPersonales?.estadosEmpleados
        : empleadoUno.idEstado,
      idEmpresadeTelefonia: 2,
      imagen: responses.formDatosPersonales?.inputImage?.substring(22)
        ? responses.formDatosPersonales?.inputImage?.substring(22)
        : empleadoUno?.imagen,
      rutaFoto: null,
      telFijo: responses.formDatosPersonales?.telefonoInput
        ? responses.formDatosPersonales?.telefonoInput
        : empleadoUno.telFijo,
      acuerdo: 0,
      neto: responses.formLiquidacion?.inputTotalNeto
        ? responses.formLiquidacion?.inputTotalNeto
        : empleadoUno.neto,
      idPaisOrigen: responses.formDatosPersonales?.paisOrigenInput
        ? responses.formDatosPersonales?.paisOrigenInput
        : empleadoUno.idPaisOrigen,
      mail: responses.formDatosPersonales?.email
        ? responses.formDatosPersonales?.email
        : empleadoUno.mail,
      telMovil: responses.formDatosPersonales?.movil
        ? responses.formDatosPersonales?.movil
        : empleadoUno.telMovil,
      tipoCuenta: responses.formDatosPersonales?.inputRadioAsidePagos
        ? responses.formDatosPersonales?.inputRadioAsidePagos
        : empleadoUno.tipoCuenta,
      totalRemuneracion: responses.formLiquidacion?.inputTotalRemu
        ? responses.formLiquidacion?.inputTotalRemu
        : empleadoUno.totalRemuneracion,
      totalNeto: responses.formLiquidacion?.inputTotalNeto
        ? responses.formLiquidacion?.inputTotalNeto
        : empleadoUno.totalNeto,
      tieneEmbargos: responses.formLiquidacion?.inputCheckEmbargo
        ? responses.formLiquidacion?.inputCheckEmbargo
        : empleadoUno.tieneEmbargos,
      tieneSumarioAdministrativo: responses.formLiquidacion
        ?.inputCheckSumAdministrativo
        ? responses.formLiquidacion?.inputCheckSumAdministrativo
        : empleadoUno.tieneSumarioAdministrativo,
      tieneLicenciaSinGoceHaberes: responses.formLiquidacion
        ?.inputCheckLicSinGoce
        ? responses.formLiquidacion?.inputCheckLicSinGoce
        : empleadoUno.tieneLicenciaSinGoceHaberes,
      obsEstudios: responses.formDatosPersonales?.observacionesEstudios
        ? responses.formDatosPersonales?.observacionesEstudios
        : empleadoUno.obsEstudios,
      obsFechaIngreso: "",
      idAgrupamiento: responses.formDatosPersonales?.inputAgrupamiento
        ? responses.formDatosPersonales?.inputAgrupamiento
        : empleadoUno.idAgrupamiento,
      idDireccion: responses.formLiquidacion?.inputDireccionLiquidacion
        ? responses.formLiquidacion?.inputDireccionLiquidacion
        : empleadoUno.idDireccion,
      idInstrumentoLegal: 2,
    };
    try {
      if (tabIndex === 0 || tabIndex === 2 || tabIndex === 8) {
        if (
          empleadoUno.iDempleado === 0 ||
          ...
          if (
            responses.fomrLiquidacion?.inputCheckAsigna &&
            !bodyPetitionEmpleadoGuarda.iDEsquema
          ) {
            return swal({
              title: "Error",
              text: "Debe seleccionar el Esquema del Empleado",
        ...
        if (
          ...
          await axios
            .post(urls.urlEmpleadoGuarda, bodyPetitionEmpleadoGuarda, {
              headers: {
          ...
          await axios
            .put(urls.urlEmpleadoGuarda, bodyPetitionEmpleadoUpdate)
            .then((res) => {
              setRefectch(!refetch);
              setSaveEmpleado(!saveEmpleado);
              swal({
          ...
          arrays[3].map(async (id) => {
            ...
            await axios
              .delete(`${urls.urlDOmicilioElimina}${id}`, {
        ...
        switch (urls) {
          case urls.urlTRabajoDelete: {
            arrays.idsTrabajosAnterioresDelete.map(async (id) => {
              await axios
                .delete(`${urls.urlTRabajoDelete}${id}`)
                .then((res) => {});
            });
          }
          case urls.urlDocDelte: {
            arrays.documentacionDelte.map(async (id) => {
              await axios.delete(`${urls.urlDocDelte}${id}`).then((res) =>
                swal({
                  title: "Ok",
          ...
          case urls.urlLicDelete: {
            arrays.licenciasDelete.map(async (id) => {
              await axios.delete(`${urls.urlLicDelete}${id}`).then((res) => {});
            });
          }
          ...
          case urls.urlDeleteFAmiliar: {
            arrays.arraysFamiliares.map(async (id) => {
              await axios
                .delete(`${urls.urlDeleteFAmiliar}${id}`)
                .then((res) => {});
            });
          }
          case urls.urlDatoExtraElimina: {
            arrays.arrayIdsDatoExtra.map(async (id) => {
              await axios
                .delete(`${urls.urlDatoExtraElimina}${id}`)
                .then((res) => {});
            });
          }
          default: {
      ...
      {localStorage.getItem("token") ? (
        <div className="container-fluid">
          ...
          <div className="d-flex flex-row-reverse  w-100 ">
            <button
              className="btn btn-danger  btnEmpl"
              onClick={() => cleanIdsGeneral()}
            >
              Cancelar
            </button>
            <button
              className="btn btn-success btnEmpl"
              onClick={() => deleteItems(objectRequest)}
            >
              Aceptar
            </button>
          </div>
